...
export class RPC extends EventEmitter {
  ...
  constructor(private readonly options: IRPCOptions) {
    ...
    this.isReady = new Promise<void>((resolve) => {
      const onLoad = options.onLoad ? options.onLoad : Promise.resolve();
  ...
  public expose<T>(
    method: string,
    handler: (params: T, origin: string) => Promise<any> | any
  ): this {
    this.on(method, (data: RPCMethodWithOrigin<T>) => {
      if (data.discard) {
        handler(data.params, data.origin);
        return;
      }

      // tslint:disable-next-line
      new Promise((resolve) => resolve(handler(data.params, data.origin)))
        .then(
          (result) =>
            ({
              type: "reply",
